Attribute VB_Name = "modOscillator"
Option Explicit
'--------------------------------------------------------------
...
Public Sub dBeep(aFrequency As Integer, aDuration As Integer, Optional aVolume As Integer)
    ...
    nFreq = 1 + aFrequency * 2.756125! * Log(1 + aFrequency / 1000) / Log(2)
    ' now call the function and play the sound
    SinBuffer nFreq, 1, 0
    
    Sleep aDuration
    ' for simulating silence, set the volume to zero and freq to 1
    SinBuffer nFreq, 0, 0
